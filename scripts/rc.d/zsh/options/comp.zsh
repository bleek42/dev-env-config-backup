...
## * This sets various options and styles for the ZSH completion system. It configures the behavior of the completion menu, including the number of completions to display, the formatting and colors of the completion list, and the handling of case sensitivity and partial word matching. It also sets up key bindings for common completion actions, and configures the behavior of the FZF fuzzy finder for file and directory completion.
## *################################################################################################
## *################################################################################################
...
unsetopt menu_complete # ! enable menu completion
unsetopt complete_aliases # ! don't expand aliases _before_ completion has finished
...
zstyle ':completion:*' completer _expand .force_rehash _complete _match _files _ignored _generic
...
if command -v fzf >/dev/null 2>&1; then
    ...
    *)
    ...
    # export FZF_DEFAULT_COMMAND='fd --type file -H -I --exclude .git'
    export FZF_DEFAULT_COMMAND="ag -l -S --hidden --ignore .git -g \""\"
    ...
    __fzf_default_preview="([[ -f {} ]] && (bat -f {} || cat {})) || ([[ -d {} ]] && (lsd --color=always --tree -A -F -d {}) || tree -C -a -f -L 2 {}) || echo {} 2> /dev/null | bat -f -p || echo {} 2> /dev/null | less -Rf"
    ...
    __fzf_colors_border="gutter:#1d1717,border:#0af2c4"
    ...
    # __fzf_default_preview_window="down:60%:hidden:border-sharp:wrap:~4"
    ...
    export FZF_DEFAULT_OPTS="
        ...
        --preview='down:60%:hidden:border-sharp:wrap:~4' \
        --preview-window='${__fzf_default_preview_window}' \
    ...
    # export FZF_CTRL_T_COMMAND="$(type -p rg &>/dev/null && echo 'rg --files --hidden --smart-case --glob "!.git/*"' || echo 'fd --hidden --type f --exclude ".git"')"
    # export FZF_CTRL_T_COMMAND='rg --files --hidden --smart-case --glob "!.git/*"'
    ...
    # export FZF_ALT_C_COMMAND=$(type -p rg &>/dev/null && echo 'rg --files --hidden --smart-case --glob "!.git/*"' || echo 'fd --hidden --type f --exclude ".git"')
    ...
    # echo "$has_fzf_tab_func"

    if [[ $(type fzf-tab-complete) == *'fzf-tab-complete is a shell function'* ]]; then
        # echo "fzf-tab keybinding detected: initializing relevant completions..."

        test -f "${FZF_TAB_HOME}/modules/Src" &&
        ...
        zstyle ':fzf-tab:complete:(kill|ps|px):argument-rest' fzf-flags \
            '-m -e -i --layout reverse --info=inline --preview-window="right:border-vertical:~4:wrap"  --pointer=" " --marker "* "'
        ...
        zstyle ':fzf-tab:complete:(-equal-:|(\\|*/|)(sudo|proxychains|strace):argument-1|pudb:option--pre-run-1)' fzf-preview \
            '[[ $group = *'external command'* ]] && command less -Rf =$word'
        ...
        zstyle ':fzf-tab:complete:(-command-:|command:option-(v|V)-rest)' fzf-preview \
            ...
            *"builtin command"*) run-help $word |  bat -f -p
            ;;
            *"external command"*) command less -Rf $word
            ;;
            *) echo ${(P)word}
            ;;
        esac'
        ...
        zstyle ':fzf-tab:complete:(\\|*/|)journalctl:*' fzf-preview \
            ...
            ;;
                *"files"*) journalctl -b /dev/$word |  bat -f -p -l=log
            ;;
               *commands*) journalctl $word | bat -f -p -l=log
            ;;
                *"possible fields"*) journalctl -u $word |  bat -f -p -l=log
            ;;
        esac'
        ...
        zstyle ':fzf-tab:complete:(\\|)zi-*' fzf-preview \
            'case $group in
                *Plugin*) bat -f -l=md ~/.local/share/zsh/plugins/$word/README*
            ;;
                *Snippet*) bat -f -p -l=md ~/.local/share/zsh/snippets/*/$word
            ;;
                *) echo "${(P)word}"
            ;;
        esac'
