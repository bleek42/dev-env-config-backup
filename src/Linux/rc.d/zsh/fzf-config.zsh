...
export FZF_DEFAULT_OPTS="
		...
		--border \
		--preview-window hidden:right:50%:border-horizontal:~4 \
		--preview '([[ -f {} ]] && (bat --color=always -n -p {} || cat {})) || ([[ -d {} ]] && (tree -aCI \"\.git\" {} | less)) || echo {} 2> /dev/null | head -200 | less {}' \
		--prompt '${FZF_PROMPT}' \
		--pointer ' ' \
		--marker '* ' \
		--select-1 \
...
FZF_TAB_OPTS="
        ...
        --header-first \
        --header '${__fzf_default_header}' \
		--preview-window down:border-vertical:~4 \
		--preview '([[ -f {} ]] && (bat -n -p --color=always {} || cat {}))
						||
...
export FZF_ALT_C_OPTS="--preview 'tree -C {} | head -200 | less'"
