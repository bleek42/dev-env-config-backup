...
# ! check if bat pkg on distro is "batcat" to make sure scripts with bat work
...
if command -v fdfind >/dev/null 2>&1; then
    ...
    alias fd='fd --hidden --no-ignore --exclude ".git" --follow --color always'
    alias fdh='fd --hidden --follow --color always'
fi
...
function set-treex() {
    if command -v exa >/dev/null 2>&1; then
        exa --all --long --group --git --links --time-style=long-iso --header --group-directories-first --color-scale --icons --tree --ignore-glob ".git" --color=always "$@" | batcat -f -n
    else
        tree -aC -I '.git' --dirsfirst "$@" | bat --color=never -n
    fi
}
