...
#   * ex.
#     export HL_COLORS_FG="bold black on_rgb520","bold red on_rgb025"
#     export HL_COLORS_BG="underline bold rgb520","underline bold rgb025"
...
hilite() {
    ...
    if [[ -n $HL_COLORS_FG ]]; then
        local _CSV="$HL_COLORS_FG"
        local OLD_IFS="$IFS"
        IFS=','
        local _COLORS_FG=()
    ...
    if [[ -n $HL_COLORS_BG ]]; then
        local _CSV="$HL_COLORS_BG"
        local OLD_IFS="$IFS"
        IFS=','
        local _COLORS_BG=()
        if [[ -n $ZSH_VERSION ]]; then
...
Check the content of your HL_COLORS_FG and HL_COLORS_BG environment variables or unset them to use default 12 defined colors."
        return 1
    fi
