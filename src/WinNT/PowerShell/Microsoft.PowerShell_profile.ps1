...
Import-Module -Name gsudoModule -Global
...
if (Test-Path $scriptAliases) {
	Get-ChildItem -Path $scriptAliases | ForEach-Object {
		. $_
	}
}
else {
	Write-Warning "Aliases.ps1 not found at $scriptAliases"
}
...
if (Test-Path $completionScriptsDir) {
	...
	Write-Warning "CompletionScriptsDir not found at $completionScriptsDir"
}
