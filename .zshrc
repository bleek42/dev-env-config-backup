...
if [[ $(command -v zi) ]]; then
    ...
    # source "${XDG_DATA_HOME:-${HOME}/.local/share}/zsh/bin/zi.zsh"
    ...
    hlissner/zsh-autopair \
        id-as='zui' \
        eval='export ZCDR="${ZI[CONFIG_DIR]}/options";' \
    z-shell/zui \
        id-as='zflai' \
    z-shell/zflai \
        id-as='ztrace' \
    ...
    z-shell/zsh-morpho \
        id-as='diff-so-fancy' \
        null \
        lbin='!bin/diff-so-fancy; bin/fancy-diff; bin/git-dsf' \
    ...
    z-shell/pyenv \
        ...
        eval='!command cp -fru /usr/share/zsh/functions/Completion/*/_* "${ZI[CONFIG_DIR]}/completions"; \
    ...
    zi wait='0c' lucid for \
        id-as='fzf-tab' \
        has='fzf' \
        atload='test -f "${ZI[CONFIG_DIR]}/options/comp.zsh" && \
                    source "${ZI[CONFIG_DIR]}/options/comp.zsh" \
                ' \
...
# else
        ...
        # eval='test -f "${ZI[CONFIG_DIR]}/options/fzf/completion.zsh" && \
        #             source "${ZI[CONFIG_DIR]}/options/fzf/completion.zsh"; \
        #         test -f "${ZI[CONFIG_DIR]}/options/fzf/keybindings.zsh" && \
        #             source "${ZI[CONFIG_DIR]}/options/fzf/keybindings.zsh"; \
        #     ' \
