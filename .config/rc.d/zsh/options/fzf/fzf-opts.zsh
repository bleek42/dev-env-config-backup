...
[[ -x $(command -v fzf) ]] || \
    echo "No FZF binary found: unable to use this script..!" && return 1
    ...
    _fzf_comprun() {
        ...
        if [[ $# -lt 1 ]]; then
            echo "Error: Expected at least 1 argument, but got $#" >&2
            return 1
        fi
        ...
        case "${command}" in
            zd | cd)
                ...
                if command -v fd &> /dev/null && command -v lsd &> /dev/null; then
                    # Call fd command to get list of directories, then pipe stdout to fzf command with preview option
                    fd --type d --hidden --follow --exclude ".git" . | \
                        fzf --preview "lsd -d -a -F -I=.git --tree --depth=2 {}" "$@"

                elif command -v fd &> /dev/null && command -v eza &> /dev/null; then
                    fd --hidden --type d --exclude ".git" . | \
                        fzf --preview "eza -d -T -I=.git -L=2 {}" "$@"

                elif command -v fd &> /dev/null && command -v exa &> /dev/null; then
                    fd --hidden --type d --exclude ".git" . | \
                        fzf --preview "exa -d -T -I=.git -L=2 {}" "$@"

                else
                    find -type d -depth 2 . | \
                        fzf --preview "ls --color=auto -a -1 -F -I .git -a {}" "$@"

                fi

                ;;
            export | set | unset | typeset)
                # Call fzf command with preview option
