...
# commands using sudo #######
    if [[ $apt_pref = nala ]]; then
        # echo "using nala aliases..."
        alias nalarm="sudo $apt_pref autoremove"
        # alias nalabd="sudo $apt_pref build-dep"
        # alias nalac="sudo $apt_pref clean"
        alias nalaupd="sudo $apt_pref update"
        alias nalaupg="sudo $apt_pref $apt_upgr --update"
        alias nalafupx="sudo $apt_pref $apt_upgr --update --fix-broken -f"
        alias nalafu="sudo apt-file update"
        # alias nalau="sudo $apt_pref $apt_upgr"
        alias nalains="sudo $apt_pref install"
        # Install all packages given on the command line while using only the first word of each line:
        # acse ... | ail

        alias nalainsls="sed -e 's/  */ /g' -e 's/ *//' | cut -s -d ' ' -f 1 | xargs sudo $apt_pref install"
        alias nalaprg="sudo $apt_pref autopurge"
        # alias nalar="sudo $apt_pref autoremove"
